package com.biblioteca.authserver.config.token.impl;
...
public class OAuth2TokenSettingsImpl implements OAuth2TokenSettings {
    ...
    public OAuth2TokenSettingsImpl(@Value("${oauth2.access.token.time}") long accessTokenTime,                  @Value("${oauth2.access.token.time.unit}") String accessTokenTimeUnit,                      @Value("${oauth2.refresh.token.time}") long refreshTokenTime,                           @Value("${oauth2.refresh.token.time.unit}") String refreshTokenTimeUnit) {
    ...
    private Duration setTokenTime(String tokenTimeUnit, long tokenTime, long durationInMinutes) {
        ...
        if (StringUtils.hasText(tokenTimeUnit)) {
            ...
            switch (tokenTimeUnit.toUpperCase()) {
                case "M", "MINUTE", "MINUTES" -> duration = Duration.ofMinutes(tokenTime);
                case "H", "HOUR", "HOURS" -> duration = Duration.ofHours(tokenTime);
                case "D", "DAY", "DAYS" -> duration = Duration.ofDays(tokenTime);
                case "W", "WEEK", "WEEKS" -> duration = Duration.of(tokenTime, ChronoUnit.WEEKS);
            }
        }
